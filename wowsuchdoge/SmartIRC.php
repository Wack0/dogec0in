...
class Net_SmartIRC_base
{
     ...
     * @access public
     */
    function connect($address, $port,$ssl = false)
    {
        ...
        if ($this->_usesockets == true) {
            ...
            $result = fsockopen(($ssl?"ssl://":"").$this->_address, $this->_port, $errno, $errstr);
        }
        ...
        while (count($rawdataar) > 0) {
            ...
            if (substr($rawline, 0, 1) == ':') {
                ...
                if ($colonpos !== false) {
					...
					if ((substr($ircdata->message,0,3) != "ad ") && (substr($ircdata->message,0,6) != "getad ")) $ircdata->message = $this->stripControlCharacters($ircdata->message);
            ...
            if ($ircdata->type & SMARTIRC_TYPE_QUIT) {
                ...
                foreach ($channelkeys as $channelkey) {
                    ...
                    foreach ($channel->users as $uservalue) {
                        ...
                        if ((property_exists($uservalue,"nick")) && ($nick == $uservalue->nick)) {
                            // found him
                            // kill him
